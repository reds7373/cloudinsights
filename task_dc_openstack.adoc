...
By default, only admin users can access the OpenStack REST APIs which the Cloud Insights data collector uses to collect data from Openstack. However, the Cloud Insights data collector can use a non-admin user role to be able to collect data from the cluster. To allow this, you must modify the policy file for the OpenStack Nova service to allow users other than admin to make the REST calls to Openstack. 

Follow these steps to allow non-admin users to make the REST API calls:

. Log in to the OpenStack controller. 
. Use a text editor to open _/etc/nova/policy.json_ and add the following lines:
...
For advanced policy configurations, refer to the link:https://docs.openstack.org/oslo.policy/latest/admin/policy-json-file.html[*Openstack documentation*]. 
