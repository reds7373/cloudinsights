...
. At the top of any performance data column (for example, _IOPS - Total_), click the "three dots" menu and select one of the Roll Up methods. Roll-up can be applied individually to each performance counter. For example, you can roll up Latency by Avg and IOPS by Sum in the same table widget.
