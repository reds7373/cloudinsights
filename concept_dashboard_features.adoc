...
Let's say we have a chart made up of data points that are collected from an object once every minute. If you are only interested in understanding hourly trends, having 60 data points in each hour's worth of data is overkill. In that case, you might aggregate the data into one-hour buckets, which would give you a single data point for each hour. You might choose to aggregate the data points into a single average number for each hour, or you might only be interested in the maximum data point that occurred during each hour. In these cases, you could choose to aggregate the data into one-hour buckets by _average_ or _maximum_. If you further want to see the averages or maximums in each of your company's data centers, you would choose to Roll Up by the _Data Center_ attribute.

Aggregating data allows charts to clean up the messiness of having too many data points while still preserving useful trends. 

==== Optimized versus Custom aggregation
...
For example, if your dashboard or widget time range is 24 hours, Cloud Insights by default will optimize your line chart data into 


==== More about Aggregation

In most cases, the minimum allowed interval is ten minutes. Note that a small interval combined with a long time range may result in an "Aggregation interval resulted in too many data points." warning. For example, you might see this if you have a small interval and increase the dashboard time frame to 7 days. In this case, Insight will temporarily increase the aggregation interval to 1 hour until you select a smaller time frame.
