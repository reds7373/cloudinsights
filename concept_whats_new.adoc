...
* Integration data (Kubernetes, ONTAP Advanced Metrics, etc.): Aggregation time interval x 2. For example, if the widget-specific aggregation interval is set at 1 minute, Cloud Insights will only show a gap if there is null data for two minutes or more. 
