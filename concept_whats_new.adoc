...
==== Roll Up Data in Table Widget

Table widgets in dashboards now allow link:concept_dashboard_features.adoc#roll-up-in-a-table-widget[roll-up of data]. You must first choose a field to "Group By", after which any columns in the table that are displaying performance data can be rolled up by Avg, Max, Min, or Sum. 
